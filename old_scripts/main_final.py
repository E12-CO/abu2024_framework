...
class MainNode(Node):
    def __init__(self):
        ...
        self.upper_white = np.uint8([179,100, 213])
...
def main(args=None):
    ...
    try:
        while rclpy.ok():
            ...
            if state <= 2 and param[0] >= 0.8 and detectable == True:
                detectable = False
                time_stamp = time.time()
                state += 1
            if state == 4 and param[1] >= 0.8 and detectable == True:
                detectable = False
                time_stamp = time.time()
                state += 1
